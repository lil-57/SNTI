  ...
  return (
    <section className="py-12 sm:py-16 bg-white">
      <div className="container px-4 sm:px-6">
        <ScrollReveal>
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">Notre Histoire</h2>
        </ScrollReveal>

        <div className="relative">
          {/* Ligne de la frise - cachée sur mobile */}
          <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-emerald-100 hidden sm:block"></div>
                ...
                <ScrollReveal delay={index * 0.1}>
                  ...
                  >
                    ...
                    <div className="w-full sm:w-full sm:max-w-md">
                      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md border border-gray-100 hover:border-emerald-200 transition-all duration-300">
                        <div className="flex items-center mb-2 sm:hidden">
                          <div className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-white font-bold text-sm mr-3">
                            {event.year}
                          </div>
                          <h3 className="text-lg font-bold">{event.title}</h3>
                        </div>
                        <h3 className="text-xl font-bold mb-2 hidden sm:block">{event.title}</h3>
                        <p className="text-gray-600 text-sm sm:text-base">{event.description}</p>
                      </div>
                    </div>

                    {/* Année - desktop seulement */}
                    <div className="absolute left-1/2 transform -translate-x-1/2 flex-col items-center hidden sm:flex">
                      <motion.div
                    ...
                    {/* Espaceur - desktop seulement */}
                    <div className="w-full sm:w-full sm:max-w-md hidden sm:block"></div>
                  </div>
                </ScrollReveal>
              </div>
            ))}
